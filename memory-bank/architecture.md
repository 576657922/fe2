...
## 步骤 3.4：错题复习模式（2025-12-09 完成）

### 功能概述

创建一个专门的沉浸式复习模式，让用户专注逐题攻克错题。与"查看错题本"的区别在于：
- **错题本**：浏览式管理，可以查看所有错题、批量标记
- **错题复习**：沉浸式学习，逐题展示、自动跳转、专注答题

### 架构设计

#### 核心组件

**1. 错题复习页面**：`/dashboard/wrong-review`
- 全屏沉浸式界面
- 单题展示模式
- 自动进度推进
- 实时进度追踪
...
用户点击"开始复习"
    ↓
调用 /api/wrong-questions 获取所有错题
    ↓
前端排序（最近错误 + 复习次数少）
    ↓
显示第一题（currentIndex = 0）
    ↓
用户答题 → 调用 /api/answers
    ↓
显示结果和解析
    ↓
2 秒后自动跳转下一题（currentIndex++）
    ↓
循环直到所有题目完成
    ↓
自动返回错题本
...
**排序策略**：
1. **第一优先级**：最近错误时间降序（最近的在前）
2. **第二优先级**：复习次数升序（复习少的在前）

**理论依据**：
- **艾宾浩斯遗忘曲线**：刚学习的内容最容易遗忘，需要及时复习
- **间隔重复学习**：确保每道题都得到充分练习
...
```typescript
// 当前题号 / 总题数
const currentQuestion = currentIndex + 1;  // 例如：3
const totalQuestions = wrongQuestions.length;  // 例如：10
const progress = Math.round((currentQuestion / totalQuestions) * 100);  // 30%
```
...
```
┌─────────────────────────────────────────┐
│ 2023_Spring | Security | 中等 | 第 15 题│
├─────────────────────────────────────────┤
│ 题干内容...                            │
│                                        │
│ ○ A. 选项 A                            │
...
**主文件**：`app/(dashboard)/dashboard/wrong-review/page.tsx` (450+ 行)
...
**错题本页面**（`wrong-book/page.tsx`）：
```tsx
{/* 头部右侧 */}
<div className="flex items-center gap-3">
  {/* 错题总数 */}
  <div>错题总数 {sortedQuestions.length}</div>

  {/* 开始复习按钮 */}
  {sortedQuestions.length > 0 && (
    <button onClick={() => router.push("/dashboard/wrong-review")}>
      开始复习
    </button>
  )}
</div>
...
**Dashboard 首页**（`dashboard/page.tsx`）：
```tsx
{/* 快速开始卡片 */}
<Link href="/dashboard/wrong-review">
  <p>错题复习</p>
  <p>专注复习错题，逐题攻克</p>
</Link>
```
...
- **API**：复用现有的 `/api/wrong-questions` 和 `/api/answers`
